...
def main():
    print("Fetching 1000 real Norwegian organizations for the benchmark...")
    try:
        url = "https://data.brreg.no/enhetsregisteret/api/enheter?size=1000"
        ...
        with open("1000-companies.jsonl", "w", encoding="utf-8") as f:
            for org in orgs:
                f.write(json.dumps({"organisation_number": org["organisasjonsnummer"]}) + "\n")
        print(f"Saved {len(orgs)} organizations to 1000-companies.jsonl")
    ...
    for stale in ["db/signalpost.db", "out/1000-envelopes.jsonl", "out/1000-profiles.jsonl", "out/1000-report.json"]:
        safe_remove(stale)
    ...
    cmd = [
        ...
        "--organisations", "1000-companies.jsonl",
        "--bulk", "brreg-enheter.csv",
        "--profiles-output", "out/1000-profiles.jsonl",
        "--output", "out/1000-envelopes.jsonl",
        "--report", "out/1000-report.json",
        "--run-id", "full-1000",
        "--expected-count", "1000"
    ]
    ...
    # Load report
    if not os.path.exists("out/1000-report.json"):
        print("ERROR: Report file out/1000-report.json not found.")
        sys.exit(1)
        
    with open("out/1000-report.json", "r") as f:
        report = json.load(f)
